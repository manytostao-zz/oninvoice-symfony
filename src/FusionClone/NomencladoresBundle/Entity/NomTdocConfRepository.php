...
class NomTdocConfRepository extends EntityRepository
{
    public function findByTdocCod($codTdoc)
    {
        $where = '';
        if (!is_null($codTdoc)) {
            $where = $where . ' AND ntc.tdoc = nt.id AND nt.codigo = :codTdoc ';


            $em = $this->getEntityManager();
            $consulta = $em->createQuery(
                'SELECT ntc
                FROM NomencladoresBundle:NomTdocConf ntc JOIN
                    NomencladoresBundle:NomTdoc nt
                WHERE 1 = 1' . $where . 'ORDER BY ntc.descripcion DESC'
            );


            $consulta->setParameter('codTdoc', $codTdoc, 'string');
    ...
    public function findByName($nombre)
    {
        if (!is_null($nombre)){
            $em = $this->getEntityManager();
            $consulta = $em->createQuery(
                'SELECT ne FROM NomencladoresBundle:NomTdocConf ne
                    WHERE ne.descripcion LIKE :nombre'
            );
    ...
    public function ajaxTable(array $get, array $filters, $flag = false)
    {
        ...
        $alias = 't';
        /* DB table to use */
        $tableObjectName = 'NomencladoresBundle:NomTdocConf';
        /**
         * Set to default
         */
        ...
        if (isset($get['search']) && $get['search']['value'] != '') {
            ...
            for ($i = 0; $i < count($get['columns']); $i++) {
                ...
                switch ($colName) {
                    default:
                        if ($colName != 'id') {
                            $aLike[] = $cb->expr()->like('t.' . $colName, '\'%' . $get['search']['value'] . '%\'');
                        }
                        break;
                }
            }
            ...
            foreach ($filters as $clave => $valor) {
                if (!is_null($valor)) {
                    switch ($clave) {
                        default:
                            if (!is_null($valor)) {
                                $aLike[] = $cb->expr()->eq('t.' . $clave, $valor);
                            }
                            break;
                    }
    ...
    public function getCount()
    {
        $query = $this->getEntityManager()
            ->getRepository('NomencladoresBundle:NomTdocConf')
